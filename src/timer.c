    ...
    if (game->split_display_titles) {
        for (unsigned int i = 0; i < game->split_count; ++i) {
            if (game->split_display_titles[i]) {
                free(game->split_display_titles[i]);
            }
        }
        free(game->split_display_titles);
        game->split_display_titles = 0;
    }
    if (game->split_is_subsplit) {
        free(game->split_is_subsplit);
        game->split_is_subsplit = 0;
    }
    if (game->split_group_names) {
        for (unsigned int i = 0; i < game->split_count; ++i) {
            if (game->split_group_names[i]) {
                free(game->split_group_names[i]);
            }
        }
        free(game->split_group_names);
        game->split_group_names = 0;
    }
    if (game->split_group_index) {
        free(game->split_group_index);
        game->split_group_index = 0;
    }
    if (game->groups) {
        for (unsigned int i = 0; i < game->group_count; ++i) {
            if (game->groups[i].name) {
                free(game->groups[i].name);
            }
        }
        free(game->groups);
        game->groups = 0;
...
int ls_game_create(ls_game** game_ptr, const char* path, char** error_msg)
{
    ...
    if (ref) {
        ...
        if (!error) {
            ...
            // Parse prefixes from titles
            unsigned int subsplit_start = UINT_MAX;
            unsigned int group_count = 0;
            for (unsigned int i = 0; i < game->split_count; ++i) {
                const char* title = game->split_titles[i];
                if (!title || !title[0]) {
                    game->split_display_titles[i] = strdup("");
                    continue;
                }
                if (title[0] == '-') {
            ...
            // Build group array: iterate again to assign group start/end
            unsigned int actual_group_count = 0;
            for (unsigned int i = 0; i < game->split_count; ++i) {
                if (game->split_group_names[i] && game->split_is_subsplit[i]) {
                    ++actual_group_count;
                }
            }
